 ...
 * Biome class, it generates a Biome, saves a Biome to a save file,
 * loads the Biome from the save file, and renders the Biome unto 
 * the screen.
 */
...
class Biome {
	...
	/**
	 * getFirstHeight
	 * This method returns the height of the right-most block of a biome
	 */
	...
	/**
	 * getLastHeight
	 * This method returns the height of the left-most block of a biome
	 */
	...
	/**
	 * removeBlock
	 * Removes the block at the given cord
	...
	/**
	 * addBlock
	 * Adds a block of the given type at the given cord
	...
	/**
	 * saveBiome
	 * Save the biome to the appropriate file
	 */
	public void saveBiome() {
		...
		try {
			...
			/* Iterate through the buffer and write the appropriate number for
			 * each block 
			 */
				...
				for (int j = 0; j < mapBuffer[i].length; j++) {
					if (mapBuffer[i][j] == null) {
						...
						switch (mapBuffer[i][j].getType()) {
							case "dirt" : output.print("1"); break; // dirt      is 1
							case "grass": output.print("2"); break; // grass     is 2
							case "leaf" : output.print("3"); break; // leaf      is 3
							case "sand" : output.print("4"); break; // sand      is 4
							case "snow" : output.print("5"); break; // snow      is 5
							case "stone": output.print("6"); break; // stone     is 6
							case "wood" : output.print("7"); break; // wood      is 7
							case "sands": output.print("8"); break; // sandstone is 8
							default     : output.print(" "); break; // null      is empty string
						}
					}
				}
				output.println();
	...
	/**
	 * loadBiome
	 * load a biome from a save file and write it into mapBuffer
	 */
	...
	public void generateBiome() {
		...
		for (int i = 0; i < Y_BUFFER_SIZE; i++) {
			for (int j = 0; j < X_BUFFER_SIZE; j++) {
				...
				// If i is smaller than the outline height and smaller than the underground outline height
	...
	/**
	 * renderBiome
	 * render the biome onto the screen
	 */
