...
/**
 * TODO save method TODO generate method
 */
...
class Biome {
	...
	/*
	 * public void renderBiome(Graphics g) { for (int i=0; i<mapBuffer.length; i++)
	 * { for (int j=0; j<mapBuffer[i].length(); j++) { switch
	 * (mapBuffer[i].charAt(j)) { case '1': (new Block(j, i, "grass" )).draw(g);
	 * break; case '2': (new Block(j, i, "dirt" )).draw(g); break; case '3': (new
	 * Block(j, i, "stone" )).draw(g); break; case '4': (new Block(j, i, "sand"
	 * )).draw(g); break; case '5': (new Block(j, i, "snow" )).draw(g); break; case
	 * '6': (new Block(j, i, "wood" )).draw(g); break; case '7': (new Block(j, i,
	 * "leaf" )).draw(g); break; case '8': (new Block(j, i, "sandstone")).draw(g);
	 * break; } } } }
	 */
}
